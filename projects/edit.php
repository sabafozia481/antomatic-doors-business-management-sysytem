<?php
$pageTitle = "Edit Project";
include '../includes/header.php';

$id = $_GET['id'] ?? null;
if (!$id) redirect('index.php');

// Fetch clients for dropdown
$stmtClients = $pdo->query("SELECT id, name FROM clients ORDER BY name ASC");
$clients = $stmtClients->fetchAll();

// Fetch project
$stmt = $pdo->prepare("SELECT * FROM projects WHERE id = ?");
$stmt->execute([$id]);
$project = $stmt->fetch();

if (!$project) redirect('index.php');

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $client_id = sanitize($_POST['client_id']);
    $project_name = sanitize($_POST['project_name']);
    $cost = sanitize($_POST['cost']);
    $status = sanitize($_POST['status']);

    if (empty($client_id) || empty($project_name) || empty($cost)) {
        $_SESSION['error'] = "All mandatory fields are required.";
    } else {
        try {
            $stmt = $pdo->prepare("UPDATE projects SET client_id = ?, project_name = ?, cost = ?, status = ? WHERE id = ?");
            $stmt->execute([$client_id, $project_name, $cost, $status, $id]);
            redirect('index.php', "Project updated successfully!");
        } catch (PDOException $e) {
            $_SESSION['error'] = "Error: " . $e->getMessage();
        }
    }
}
?>

<div class="data-card" style="max-width: 600px; margin: 0 auto;">
    <div class="card-header">
        <h2>Update Project Details</h2>
    </div>
    <div style="padding: 24px;">
        <form action="edit.php?id=<?php echo $id; ?>" method="POST">
            <div class="form-group">
                <label for="client_id">Select Client *</label>
                <select id="client_id" name="client_id" required>
                    <?php foreach ($clients as $client): ?>
                        <option value="<?php echo $client['id']; ?>" <?php echo $client['id'] == $project['client_id'] ? 'selected' : ''; ?>>
                            <?php echo $client['name']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                <label for="project_name">Project Name *</label>
                <input type="text" id="project_name" name="project_name" value="<?php echo $project['project_name']; ?>" required>
            </div>
            <div class="form-group">
                <label for="cost">Estimated Cost ($) *</label>
                <input type="number" id="cost" name="cost" step="0.01" value="<?php echo $project['cost']; ?>" required>
            </div>
            <div class="form-group">
                <label for="status">Project Status</label>
                <select id="status" name="status">
                    <option value="Pending" <?php echo $project['status'] === 'Pending' ? 'selected' : ''; ?>>Pending</option>
                    <option value="Completed" <?php echo $project['status'] === 'Completed' ? 'selected' : ''; ?>>Completed</option>
                </select>
            </div>
            <div style="display: flex; gap: 12px; margin-top: 32px;">
                <button type="submit" class="btn btn-primary">Update Project</button>
                <a href="index.php" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>

<?php include '../includes/footer.php'; ?>
